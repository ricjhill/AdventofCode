...
the binary number is padded with leading zeroes until its length is a multiple of four bits, and then it is broken into
 groups of four bits. Each group is prefixed by a 1 bit except the last group, which is prefixed by a 0 bit.
...
input_data: str = ''.join(chain(*Path('day16_input.txt').read_text().rstrip().split('\n')))
data = bin(int(input_data, base=16))[2:]
...
def compute(dat):
    ...
    "Read the  version and type from the initial packet "
    versions.append(int(dat[pos:pos + 3], 2))
    pos += 3

    types.append(int(dat[pos:pos + 3], 2))
    pos += 3
    "Process the operator packets which all have  type !=4 "
    if types[-1] != 4:
